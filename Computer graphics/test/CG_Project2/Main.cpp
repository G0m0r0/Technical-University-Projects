...
int main()
{
	...
	try
	{
		...
		Vertext vertices[] = {
			...
			Vertext(-0.3f, -0.3f, 0.0f, .00f, .50f, .20f, 0.0f, 0.f ), //tr
			Vertext( 0.3f, -0.3f, 0.0f, .00f, .50f, .20f, 1.f, 0.f ), //br
			Vertext(-0.3f,  0.3f, 0.0f, .00f, .50f, .20f, 0.0f, 1.f), //bl
			Vertext( 0.3f,  0.3f, 0.0f, .00f, .50f, .20f, 1.0f, 1.f ) //tl
		};
...
// LATER 1 END
		...
		{
			std::shared_ptr<unsigned char> pData = std::shared_ptr<unsigned char>(stbi_load("Resources/container.jpg", &width, &height, &nrChannels, 0), stbi_image_free);
			if (!pData)
