...
public:
	...
	{//check if snake collided with food
		if (snake.XMax() > food.XMin() && snake.XMin() < food.XMax()
			...
			//food.SetPosition(glm::vec3(0.0f, 0.0f, -0.5f));

			if (snakeList.size() == 0) {
				snake.SetOldPosition(snake.Position());
			}
			else if (snakeList.size() >= 1) {

				snakeList.at(snakeList.size() - 1).get()->SetOldPosition(snakeList.at(snakeList.size() - 1).get()->Position());
			}
